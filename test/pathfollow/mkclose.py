#!/usr/bin/env python3
"""Close a DAMAGED cohesive facet and verify the normal law directly.
...
CLOSE_DOC = """
** This deck exists to verify the NORMAL law of cohesive_uc6.f directly,
** not through a structural response.  Step 1 pulls the single facet onto
** its softening branch and stops short of failure; step 2 pushes the same
** facet back through zero separation and into contact.
**
** Everything needed to check the law point by point is printed every
** increment: the interface separation from U at the two coincident node
** sets, the damage from SDV, and the traction from S on the facet.  The
** identity checked by check_close.py is
**
**   sharp law   T = kn*d            for d < 0
**               T = g*kn*d          for d >= 0
**   blended     T = g*kn*d - (1-g)*kn*psi(-d)
**               psi(u) = 0, u<=0 ; u^2/(2w), 0<u<w ; u-w/2, u>=w ; w = zeta*d0
**
** with g = max(gmin, 1-dvisc) read from SDV 2.  No bulk model, no finite
** strain correction and no closed-form structural branch enters it, so the
** test is on the constitutive routine and nothing else.
"""
...
def build_close(a):
    ...
    for ln in lines:
        if ln.startswith("*STEP"):
            break
        out.append(ln)
    # interface node sets, so the separation can be read from U
    minus = [ "%d" % (a.isplit * 4 + 1 + j) for j in range(4) ]
    out.append("** the coincident interface nodes: minus side, then the")
    out.append("** duplicates the right block uses.")
    out.append("*NSET, NSET=NIFM")
    out.append(", ".join(str(x) for x in a._iface_m))
    out.append("*NSET, NSET=NIFP")
    out.append(", ".join(str(x) for x in a._iface_p))
    out.append(CLOSE_DOC.rstrip())
    def step(name, uend, dtmax, inc):
        out.append("*STEP, INC=%d, NLGEOM" % inc)
        out.append("*STATIC")
        out.append("%g, 1.0, %g, %g" % (dtmax, a.dtmin, dtmax))
        out.append("*BOUNDARY")
        out.append("NRIGHT, 1, 1, %g" % uend)
        out.append("*NODE PRINT, NSET=NIFM, FREQUENCY=1")
        out.append("U")
        out.append("*NODE PRINT, NSET=NIFP, FREQUENCY=1")
        out.append("U")
        out.append("*EL PRINT, ELSET=ECOH, FREQUENCY=1")
        out.append("S, SDV")
        out.append("*END STEP")
    step("pull", a.upull, a.dtpull, 2000)
    step("close", -a.uclose, a.dtclose, 4000)
    return "\n".join(out) + "\n"
...
def main():
    ...
    p.add_argument("--upull", type=float, default=1.2e-2,
                   help="end displacement of step 1: damages the facet and "
                        "stops short of failing it")
    p.add_argument("--uclose", type=float, default=1.2e-1,
                   help="how far into compression step 2 pushes")
    ...
    a = p.parse_args()
    # the interface node numbers, recomputed the way build() assigns them
    n = 0; nid = {}
    for i in range(a.nslice + 1):
        for j in (0, 1):
            for k in (0, 1):
                n += 1; nid[(i, j, k)] = n
    a._iface_m = [nid[(a.isplit, j, k)] for j in (0, 1) for k in (0, 1)]
    a._iface_p = [n + 1 + t for t in range(4)]
    open(a.out, "w").write(build_close(a))
    d0 = a.tn0 / a.kn; df = 2.0 * a.gc / a.tn0
    print("wrote %s: d0=%.6g df=%.6g, step 1 to u=%g, step 2 to u=%g"
          % (a.out, d0, df, a.upull, -a.uclose))
