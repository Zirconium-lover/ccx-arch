...
/* [DAMAGE RESCUE] owned by nonlingeo.c.  All three are 0 unless
   ...
   routine (electromagnetics.c) is untouched.
     ccx_rescue_active : rescue is enabled at all (for logging)
	...
	*itp=1;
	...
	if(*idrct==1){
	  ...
	  *istab=0;
	  ...
	  if(*dtheta<*tmin){
	    /* ---- [DAMAGE RESCUE] the terminal wall -------------------
	       The stock code below writes the frd and stops the process.
	       When rescue is armed, defer that once: signal nonlingeo
	       and leave exactly as an ordinary cutback leaves, so its
	       existing rollback restores the start of this increment. */
	    if((ccx_rescue_arm==1)&&(ccx_rescue_req==0)){
	      printf("[DAMAGE RESCUE] WALL inc=%" ITGFORMAT " iter=%"
	             ITGFORMAT " time=%.12e dtime=%.12e reason=divergence-cutback-below-tmin"
	             " dtheta_next=%.12e tmin=%.12e -> stop DEFERRED,"
	             " handing the increment to the STANDARD cutback"
	    ...
	    if(ccx_rescue_active==1){
	      printf("[DAMAGE RESCUE] the rescue attempt did not"
	             ...
	             " unchanged - no damped and no minimal step is"
	             " accepted\n");
	      fflush(stdout);
	    }
	    printf("\n *ERROR: increment size smaller than minimum\n");
	...
	if(*idrct!=1){
	  ...
	  *istab=0;
	  ...
	  if(*dtheta<*tmin){
	    ...
	    if((ccx_rescue_arm==1)&&(ccx_rescue_req==0)){
	      printf("[DAMAGE RESCUE] WALL inc=%" ITGFORMAT " iter=%"
	             ITGFORMAT " time=%.12e dtime=%.12e reason=too-slow-cutback-below-tmin"
	             " dtheta_next=%.12e tmin=%.12e -> stop DEFERRED,"
	             " handing the increment to the STANDARD cutback"
	             " rollback for one rescue attempt\n",
	             *iinc,*iit,*time,*dtime,*dtheta,*tmin);
