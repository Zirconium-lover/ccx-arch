...
| **the rank-1 PROJECTION in `mafilldamas.f` as the place the term is lost** | refuted. Extracted to `src/damrank1.f` and checked three ways: against `e_c3d`'s own quadruple sum at `F != I` (`4.9e-16` of the block's largest entry), against a finite difference of the element internal force at `F = I` (`1.1e-10`), and deliberately broken by doubling the shear slots of `dD/d(eps)` - which the test catches at `0.25` relative. The projection is right; the earlier "moves the worst coefficient by 7.13 against a gap of 1375" stands as a measurement and no longer supports that conclusion |
| **`CCX_DAMAGE_TANGENT=FD_SYM`** - now **DELETED** | it had two convention defects (`dD/d(eps)` in tensor shear against an engineering-shear tangent, and no viscous `beta`), which is why it destroyed `fast-plain` at increment 41 with zero deletions. **Corrected**, it runs - and costs **1447 Newton iterations against 1286 with no correction at all** (+13%), 25.62 s against 23.11 s, and moves the fracture by one element. The symmetric part of a nonsymmetric operator is a third operator, not a tangent. Deleted rather than disabled: `research/06-TANGENT-VERDICT.md` |
| **`CCX_DAMAGE_TANGENT=UNSYM` as worth its cost** | on `fast-plain` it produces a **byte-identical** `m.damage` and buys **0.2%** of Newton iterations for **+37% of wall time** (23.11 s -> 31.58 s). On `fast-wrapped` it walls at the same `theta` with the same 64 elements and takes **6% more** iterations. It stays, off by default, because it is the only mode that is a Newton method and `s3rad` has not been measured |
| **the tangent as the cause of the first wall** | four modes - stock, `UNSYM`, corrected `FD_SYM`, `UNSYM`+`TANGENT_FULL` - wall on `fast-wrapped` at `theta` 0.15875 to 0.15877 with the **identical 64-element deletion set**. A correct Newton direction does not move that wall |
| **`CCX_DAMAGE_TANGENT_FULL=1`** (move the tangent cut-off from `D=0.999` to the residual-stiffness floor, closing the one hole `mafilldamas.f` calls genuine) | **output identical to leaving it off** on `fast-wrapped`. The terminal band it serves is empty on both fast decks. Not deleted only because `s3rad` has 3790 deletions against 90 |
