...
_set_output_format(_TWO_DIGIT_EXPONENT);
  ...
  printf("************************************************************\n\n");
  printf("You are using an executable made on Thu Aug 20 17:18:08 MSK 2026\n");
  fflush(stdout);
  ...
  while(istat>=0){
	...
	if(iperturb[0]<2){
	  ...
	  /* The fracture branch implements user elements (the UC6 zero-
	     thickness cohesive facet) only on the nonlinear path in
	     nonlingeo.c.  A purely linear step reaches linstatic.c, which
	     has no UC6 handling at all, and dies with an access violation
	     before the first iteration.  Fail with a diagnosis instead.
	     Reproducer: any UC6 deck whose materials are elastic only and
	     whose *STEP card has no NLGEOM. */
	  for(i=0;i<ne;i++){
	    if(lakon[8*i]=='U'){
	      printf(" *ERROR in CalculiX: element %" ITGFORMAT " is a user\n",i+1);
	      printf("        element, and user elements are implemented\n");
	      printf("        only on the nonlinear solution path of this\n");
	      printf("        build.  Add NLGEOM to the *STEP card, or give\n");
	      printf("        a material a nonlinear response, so that the\n");
	      printf("        nonlinear solver is used.\n");
	      FORTRAN(stop,());
	    }
	  }
